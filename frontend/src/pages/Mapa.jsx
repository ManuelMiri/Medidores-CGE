...
// Captura click en el mapa para agregar nuevo punto
...
export default function Mapa() {
  ...
  // Formulario nuevo/editar medidor
  ...
  async function cargarUls() {
    try {
      ...
      setUls(data.uls)
      // Por defecto activa la primera UL
      if (data.uls.length > 0) setUlsActivas([data.uls[0]])
    } catch {
      setError('Error al cargar las unidades de lectura')
    } finally {
      setCargando(false)
    }
  }
  ...
  async function cargarMedidores() {
    try {
      setCargando(true)
      const promesas = ulsActivas.map(ul =>
        api.get(`/medidores?ul=${ul}&limite=500`)
      )
  ...
  async function handleBuscar(e) {
    ...
    try {
      ...
      if (data.medidores.length > 0) {
        ...
        alert('No se encontró ningún medidor')
      }
  ...
  async function handleEliminar(instalacion) {
    if (!confirm(`¿Eliminar el medidor ${instalacion}? Esta acción no se puede deshacer.`)) return
    try {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      <nav className="navbar navbar-dark bg-primary px-3 py-2">
        ...
        <div className="d-flex align-items-center gap-3">
          <span className="text-white small">👤 {usuario.nombre} · {usuario.rol}</span>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '1rem' }}>
            ...
            {modoAgregar && (
              <p style={{ fontSize: '0.75rem', color: '#718096', marginTop: '0.5rem', textAlign: 'center' }}>
                Haz click en el mapa para colocar el nuevo punto
              </p>
            )}
          </div>
        </div>
        ...
        <div style={{ position: 'relative', flex: 1 }}>
          ...
          {modoAgregar && (
            <div style={{
              ...
              📍 Modo agregar activo — click en el mapa
            </div>
          )}
          ...
          {cargando && (
            <div style={{
              ...
              <Spinner animation="border" size="sm" /> Cargando medidores...
            </div>
          )}

          <MapContainer
            center={CENTRO_MAULE} zoom={13}
                  ...
                  <Popup minWidth={220}>
                    <div>
                      ...
                      <p style={{ margin: 0, fontSize: '0.82rem' }}>
                        ...
                        {m.observaciones && (
                          <><strong>Obs:</strong> {m.observaciones}<br /></>
                        )}
                      </p>
        ...
        {/* Panel formulario lateral derecho */}
        {formulario && (
          <div style={{
            ...
            {nuevoPunto && (
              <div className="alert alert-success py-1 px-2 mb-2" style={{ fontSize: '0.8rem' }}>
                📍 Lat: {nuevoPunto[0].toFixed(6)}, Lng: {nuevoPunto[1].toFixed(6)}
              </div>
            )}
            ...
            <div className="mb-2">
              <label style={{ fontSize: '0.8rem', fontWeight: '600' }}>UL (Unidad de Lectura)</label>
              <select
            ...
            >
              {guardando ? <Spinner size="sm" /> : (medidorEdit ? '💾 Guardar cambios' : '➕ Agregar medidor')}
            </button>
          </div>
        )}
      </div>
